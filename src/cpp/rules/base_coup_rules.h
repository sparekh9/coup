...
// ============================================================================
// BASE COUP RULES (Simplified 3-card version)
// ============================================================================
...
struct BaseCoupRules {
    ...
    static constexpr int DECK_SIZE = 6;  // 2 of each card type
    ...
    static Influence get_required_influence(Action action) {
        switch (action) {
            ...
            default: return Influence::DUKE;  // Fallback
        }
    }
    ...
    static void populate_legal_actions(const GameStateType& state,
                                       ActionList<BaseCoupRules>& result) {
        result.count = 0;
    ...
    static void execute_action(GameStateType& state, int player_id, Action action) {
        switch (action) {
            case Action::INCOME:
                // Already handled in apply_action (unchallenged action)
                break;
    ...
    static bool should_force_challenge(const GameStateType& state, Action pending_action) {
        ...
        // Rule 4: One held + one revealed → only CHALLENGE
